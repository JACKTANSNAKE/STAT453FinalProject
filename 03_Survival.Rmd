...
```{r}
Dlnorm <- survreg(Surv(dtime, death) ~ 1 , dist='lognormal', data=rotterdam)
Dlnorm
DLweibull <- survreg(Surv(dtime, death) ~ 1 , dist='weibull', data=rotterdam)
DLweibull
CS_LnormD <- -log(1 - plnorm(rotterdam$dtime,  8.318665, 1.206342))
CS_WeibullD <- -log(1 - pweibull(rotterdam$dtime,  1/0.7977553, exp(8.577572)))
...
TODO: why log-normal fit.

cumulative hazard?

```{r}
library(bshazard)
fit <- bshazard(Surv(dtime, death) ~ 1, data=rotterdam)
KM <- survfit(Surv(dtime, death) ~ 1, data=rotterdam)
plot(fit, conf.int = FALSE)
```

```{r}
meanlog <- 8.514204484-0.382172447*(rotterdam$Treatment=="Chemo")
                                                  -0.479063703*(rotterdam$Treatment=="Hormon")   
                                                  -0.605193863*(rotterdam$Treatment=="NaN/Other Treatment")
                                                  -0.458345796*(rotterdam$size=="20-50")
                                                  -0.738657689*(rotterdam$size==">50")
...
hlnorm = function(x,meanlog,sdlog) { dlnorm(x,meanlog=meanlog,sdlog=sdlog)/(1-plnorm(x,meanlog=meanlog,sdlog=sdlog)) }
plot( hlnorm(rotterdam$rtime,meanlog=meanlog,sdlog=sdlog))
```


TODO: explain why age in `rtime` models is positive: competing risks.

In the `rtime ~ Treatment + size + nodes + age` model, it is surprised to see that the coefficient of `age` is positive, though the value is small. One possible explanation could be the idea of "competing events" and "competing risk".
...
```{r}
new = data.frame(Treatment = "Both", size = "<=20", nodes=0, age=0)
plot(survfit(m_death_withage, newdata = new),fun="cumhaz", conf.int = FALSE, ylim = c(0,0.25))
abline(0, 0.1/4000, col="blue")
```
...
```{r}
m_recur = coxph(Surv(rtime, recur) ~ strata(Treatment) + strata(size) + nodes + age, data=rotterdam)
m_recur
cox.zph(m_recur)
```
