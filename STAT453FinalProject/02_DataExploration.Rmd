...
`size` v.s. `grade`
```{r}
ggplot(data = rotterdam, aes(x = as.factor(grade), fill = size)) +
  geom_bar(alpha = 0.7)
```

`grade` v.s. `nodes`
#```{r}
ggplot(data = rotterdam, aes(x=as.factor(grade), y=nodes, fill=as.factor(grade))) +
    geom_violin() +
...
`size` v.s. `nodes`
#```{r, warning = FALSE}
ggplot(data = rotterdam, aes(x=size, y=nodes, fill=size)) +
    geom_violin() +
    scale_fill_viridis(discrete = TRUE, alpha=0.6) +
    geom_jitter(color="black", size=0.4, alpha=0.9) +
    theme_ipsum() +
...
```{r}
ggplot(rotterdam, aes(x=age)) +
  geom_histogram(color="white")
```

```{r}
ggplot(rotterdam, aes(y=pgr, x=age)) + 
    geom_point() 
```

```{r}
ggplot(rotterdam, aes(y=er, x=age)) + 
    geom_point() 
```
...
```{r}
ggplot(rotterdam, aes(x=size, fill=Treatment))+
  geom_bar(position = "fill")
```

```{r}
ggplot(rotterdam, aes(x=grade, fill=Treatment))+
  geom_bar(position = "fill")
```

```{r}
ggplot(rotterdam, aes(x=age, fill=Treatment))+
  geom_bar(position = "fill")
```
